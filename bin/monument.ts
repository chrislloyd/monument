...
async function processFile(filePath: string) {
  ...
  const processDoc = spawn('bun', ['src/process-doc.ts', filePath], { stdio: ['ignore', 'pipe', 'ignore'] });
  const docClearLog = spawn('bun', ['src/clear-log.ts', tempPath], { stdio: ['pipe', 'ignore', 'ignore'] });
  processDoc.stdout.pipe(docClearLog.stdin);

  const watchAi = spawn('bun', [
    'src/watch-ai.ts',
  ...
  const aiClearLog = spawn('bun', ['src/clear-log.ts', outputPath], { stdio: ['pipe', 'ignore', 'ignore'] });
...
try {
  ...
  for await (const event of watch(inputDir, { recursive: true })) {
    if (event.filename) {
      if (event.filename.includes(outputDir)) { continue; }
